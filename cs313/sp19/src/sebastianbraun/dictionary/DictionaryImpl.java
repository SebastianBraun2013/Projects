...
/**
 * This class uses FixedSizeList to hold instances of the HashTableSlot to
...
public class DictionaryImpl<Key extends Number, E> implements Dictionary<Key, E> {
    ...
    //Constructor creates a list of size = primeNumber > start, fills the list
    //with HashTableSlot and converts to a FixedSizeList
    ...
    public E find(Key k) {
        ...
        int hashKey = getHash(k, i);
        HashTableSlot<Key, E> entry = this.F.get(hashKey);
        //if while loop results in a NullPointerException then k != any keys in 
        // the hashtable
        try {
            while (!entry.getKey().equals(k)) {
                i++;
